...
void RelCacheTable::recordToRelCatEntry(union Attribute record[RELCAT_NO_ATTRS], RelCatEntry* relCatEntry) {
    // copy values from record to relCatEntry fields
    strcpy(relCatEntry->relName, record[RELCAT_REL_NAME_INDEX].sVal); // Relation Name
    relCatEntry->numAttrs = (int)record[RELCAT_NO_ATTRIBUTES_INDEX].nVal; // #Attributes
    relCatEntry->numRecs = (int)record[RELCAT_NO_RECORDS_INDEX].nVal; // #Records
    relCatEntry->firstBlk = (int)record[RELCAT_FIRST_BLOCK_INDEX].nVal; // First Block
    relCatEntry->lastBlk = (int)record[RELCAT_LAST_BLOCK_INDEX].nVal; // Last Block
    relCatEntry->numSlotsPerBlk = (int)record[RELCAT_NO_SLOTS_PER_BLOCK_INDEX].nVal; // Slots Per Block
}
...
int RelCacheTable::resetSearchIndex(int relId) {
    if(relId < 0 || relId >= MAX_OPEN) { // check for out of bound relId
        return E_OUTOFBOUND;
    }

    if(relCache[relId] == nullptr) return E_RELNOTOPEN; // check if relation is open
